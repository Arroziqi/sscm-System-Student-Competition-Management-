...
export class CompetitionService {
  ...
  static async get(user: User, id: number): Promise<CompetitionResponse> {
    ...
    const competition = await CompetitionServiceUtil.checkCompetitionExistence(
      id,
      user.username
    );
  ...
  static async update(
    ...
    await CompetitionServiceUtil.checkCompetitionExistence(
      updateRequest.id,
      user.username
    );
  ...
  static async search(
    user: User,
    request: SearchCompetitionRequest
  ...
  static async list(user: User): Promise<Array<CompetitionResponse>> {
    const competitions = await prismaClient.competition.findMany({
      where: {
        username: user.username,
      },
    ...
    return competitions.map((comp) => toCompetitionResponse(comp));
  }
}
